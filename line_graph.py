...
def plot_original_graph(x_values, y_values, height, width, x_max, y_max, x_offset, y_offset):
    ...
    df_modified = pd.DataFrame({'x': modified_x_values, 'y': modified_y_values})

    # Print the DataFrame
    print("Predicted Dataframe:")
    print(df_modified)
    plt.plot(modified_x_values, modified_y_values, marker='o', linestyle='-', color='blue')
...
def main():
    ...
    if x_values_filtered and y_values_filtered:
        plot_original_graph(x_values_filtered, y_values_filtered, height, width, 11, 30, 0, 0)
